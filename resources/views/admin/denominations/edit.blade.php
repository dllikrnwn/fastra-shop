...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="flex items-center gap-3 mb-8">
        ...
        <h1 class="section-title">Edit Harga: {{ $denomination->name }}</h1>
    </div>

    <form method="POST" action="{{ route('admin.denominations.update', $denomination) }}" class="space-y-5">
        @csrf @method('PUT')
        <div class="nb-card-solid p-6 space-y-5">
            <div>
                ...
                <select name="game_id" class="nb-input" required>
                    @foreach($games as $g)
                    <option value="{{ $g->id }}" {{ old('game_id', $denomination->game_id) == $g->id ? 'selected' : '' }}>{{ $g->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $denomination->name) }}" class="nb-input" required>
                ...
                <div>
                    ...
                    <input type="text" name="nominal" value="{{ old('nominal', $denomination->nominal) }}" class="nb-input" required>
            ...
            <div class="grid grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="text" inputmode="numeric" name="price" value="{{ old('price', $denomination->price) }}" class="nb-input" min="0" required>
                ...
                <div>
                    ...
                    <input type="number" name="sort_order" value="{{ old('sort_order', $denomination->sort_order) }}" min="0" class="nb-input">
                </div>
                <div class="flex items-end pb-1">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="hidden" name="is_active" value="0">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $denomination->is_active) ? 'checked' : '' }} class="w-4 h-4 rounded border-gray-300 dark:border-gray-600 text-accent focus:ring-accent/20">
        ...
        <div class="flex gap-3">
            ...
            <button type="submit" class="nb-btn-primary flex-1 shadow-glow-sm hover:shadow-glow">Update Harga</button>
        </div>
    </form>
</div>
@endsection