...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="flex items-center gap-3 mb-8">
        ...
        <h1 class="section-title">Tambah Game Baru</h1>
    </div>
    ...
    <form method="POST" action="{{ route('admin.games.store') }}" enctype="multipart/form-data" class="space-y-5">
        ...
        <div class="nb-card-static p-6 space-y-5">
            <div>
                ...
                <input type="text" name="name" value="{{ old('name') }}" class="nb-input" required placeholder="Contoh: Mobile Legends">
            ...
            <div>
                ...
                <input type="file" name="image" accept="image/*" class="nb-input file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-[2px] file:border-black file:text-sm file:font-bold file:cursor-pointer" required>
            ...
            <div>
                ...
                <p class="text-xs mt-1" style="color: var(--text-secondary);">Format: MP4, WebM. Maks 50MB. Opsional.</p>
            <div>
                <label class="block text-sm font-display font-bold mb-1.5" style="color: var(--text-primary);">Deskripsi</label>
                <textarea name="description" class="nb-input" rows="3" placeholder="Deskripsi singkat game...">{{ old('description') }}</textarea>
            </div>
            <div class="grid grid-cols-2 gap-4">
                ...
                <div class="flex items-end gap-6 pb-1">
                    <label class="flex items-center gap-2 cursor-pointer">
                        ...
                        <input type="checkbox" name="is_featured" value="1" {{ old('is_featured') ? 'checked' : '' }} class="w-4 h-4 accent-[#00E5FF]">
                        <span class="text-sm font-bold" style="color: var(--text-primary);">Featured</span>
                    </label>
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="hidden" name="is_active" value="0">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', 1) ? 'checked' : '' }} class="w-4 h-4 accent-[#00E5FF]">
        ...
        <div class="flex gap-3">
            ...
            <button type="submit" class="nb-btn-primary flex-1">Simpan Game</button>
        </div>
    </form>
</div>
@endsection
